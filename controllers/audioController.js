...
const analyzeSentiment = async (text) => {
  const client = new TextAnalyticsClient(TEXT_ANALYTICS_ENDPOINT, new AzureKeyCredential(TEXT_ANALYTICS_KEY));
...
const analyzeAudioFile = async (req, res) => {
  if (!req.file) return res.status(400).send("Lütfen bir ses dosyası yükleyin!");

  const audioFilePath = path.resolve(__dirname, "../uploads", req.file.filename);
  ...
  try {
    ...
    const resultFilePath = path.resolve(__dirname, "../uploads/audio_analysis_results.json");
    const result = { sentiment_analysis: sentimentResult, mood_advice: moodAdvice };
    ...
    console.error("Ses Analizi Hatası:", error.message);
    res.status(500).json({ error: "Ses analizi sırasında hata oluştu.", details: error.message });
  }
};
